...
const createGame = (roomId, creatorName) => {
  return {
    ...
    winner: null,
    guessHistory: [],
    secrets: new Map(), // playerId -> secret
    playerSecrets: {}, // for client visibility
    creatorName,
    maxGuesses: 20,
  };
};
...
io.on("connection", (socket) => {
  ...
  socket.on("make-guess", ({ targetPlayerId, guess }) => {
    ...
    const digitResults = guessDigits.map((guessDigit, pos) => ({
      position: pos,
      guessDigit,
      targetDigit: targetDigits[pos],
      correct: guessDigit === targetDigits[pos],
      hint:
        guessDigit === targetDigits[pos]
          ? "Correct!"
          : guessDigit < targetDigits[pos]
          ? "Higher"
          : "Lower",
    ...
    if (allCorrect) {
      ...
      console.log(
        `Game ${game.id} ended! Winner: ${guesser.name} guessed ${target.name}'s number!`
      );
    } else {
